...
def update_clock():
    ...
    for i in range(60):
        ...
        if i % 5 == 0:
            ...
            canvas.create_line(x1, x2, y1, x2, fill="black",width=1)
    ...
    minute_angel = (minute + second/60) * math.pi/30 -math.pi/2
    minute_x = WIDTH/2 + 0.7 * WIDTH/2 * math.cos(minute_angel)
    minute_y = WIDTH/2 + 0.7 * WIDTH/2 * math.sin(minute_angel)
